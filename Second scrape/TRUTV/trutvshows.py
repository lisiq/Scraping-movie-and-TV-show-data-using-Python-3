...
urlshows = 'https://www.trutv.com/shows'
...
for i in episodes:
    with open('/home/lisi/MediaBiz/Second scrape/TRUTV/episodes.txt', 'a') as f:
        f.write(i+'\n')


with open('/home/lisi/MediaBiz/Second scrape/TRUTV/episodes.txt', 'r') as file:
...
for i in lines:
    ...
    c = r.content
    soup = BeautifulSoup(c, 'html.parser')
    with open('/home/lisi/MediaBiz/Second scrape/TRUTV/TRUTVshows_ldjson/' + i.encode("utf-8").hex() + '.json', 'w') as j:
        j.write(json.dumps(
...
# for i in lines:
#     try:
#         with open('/home/lisi/MediaBiz/Second scrape/TRUTV/TRUTVshows_ldjson/' + i.encode("utf-8").hex() + '.json', 'r') as j:
...
# trutvshows_ldjson = pd.DataFrame(flat_list, 
...
# trutvshows_ldjson.to_csv('/home/lisi/MediaBiz/Second scrape/TRUTV/trutvshows_ldjson.csv', index=False)
...
for i in lines:
    ...
    c = r.content
    soup = BeautifulSoup(c, 'html.parser')
    with open('/home/lisi/MediaBiz/Second scrape/TRUTV/TRUTVshows_drupla-settings-json/' + i.encode("utf-8").hex() + '.json', 'w') as j:
        j.write(json.dumps(
            str(soup.find_all(lambda tag: tag.name == 'script' and 
...
# for i in lines:
#     try:
#         with open('/home/lisi/MediaBiz/Second scrape/TRUTV/TRUTVshows_drupla-settings-json/' + i.encode("utf-8").hex() + '.json', 'r') as j:
#             dict_list.append(dict(eval(json.load(j).replace('null', '"NaN"')\
#                                        .replace('true',"True")\
...
# # create a dataframe
# trutvshows_drupla_settings_json = pd.DataFrame(flat_list)

# # finally save it
# trutvshows_drupla_settings_json.to_csv('/home/lisi/MediaBiz/Second scrape/TRUTV/trutvshows_drupla_settings_json.csv', index=False)

# # save the merger
# df_trutvshows = pd.merge(trutvshows_drupla_settings_json, trutvshows_ldjson, how='left', on='url')

# df_trutvshows.to_csv('/home/lisi/MediaBiz/Second scrape/TRUTV/trutvshows.csv', index=False)
