...
for i in result:
    with open('/home/lisi/MediaBiz/TNTDRAMAmovies/results.txt', 'a') as f:
        f.write(i+'\n')
...
for i in result:
    ...
    c = r.content
    soup = BeautifulSoup(c, 'html.parser')
    with open('/home/lisi/MediaBiz/TNTDRAMAmovies/TNTDRAMAmovies_ldjson/' + i.encode("utf-8").hex() + '.json', 'w') as j:
        j.write(json.dumps(
...
for i in result:
    ...
    c = r.content
    soup = BeautifulSoup(c, 'html.parser')
    with open('/home/lisi/MediaBiz/TNTDRAMAmovies/TNTDRAMAmovies_drupla-settings-json/' + i.encode("utf-8").hex() + '.json', 'w') as j:
        j.write(json.dumps(
            str(soup.find_all(lambda tag: tag.name == 'script' and 
