...
for i in episodes:
    with open('/home/lisi/MediaBiz/TRUTV/episodes.txt', 'a') as f:
        f.write(i+'\n')
...
for i in lines:
    ...
    c = r.content
    soup = BeautifulSoup(c, 'html.parser')
    with open('/home/lisi/MediaBiz/TRUTV/TRUTVshows_ldjson/' + i.encode("utf-8").hex() + '.json', 'w') as j:
        j.write(json.dumps(
...
for i in new_lines:
    ...
    c = r.content
    soup = BeautifulSoup(c, 'html.parser')
    with open('/home/lisi/MediaBiz/TRUTV/TRUTVshows_drupla-settings-json/' + i.encode("utf-8").hex() + '.json', 'w') as j:
        j.write(json.dumps(
            str(soup.find_all(lambda tag: tag.name == 'script' and 
