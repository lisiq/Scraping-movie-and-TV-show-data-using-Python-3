...
for i in result:
    with open('/home/lisi/MediaBiz/TBSmovies/results.txt', 'a') as f:
        f.write(i+'\n')
...
for i in result:
    ...
    c = r.content
    soup = BeautifulSoup(c, 'html.parser')
    with open('/home/lisi/MediaBiz/TBSmovies/TBSmovies_ldjson/' + i.encode("utf-8").hex() + '.json', 'w') as j:
        j.write(json.dumps(
...
# for i in result:
#     try:
#         with open('/home/lisi/MediaBiz/TBSmovies/TBSmovies_ldjson/' + i.encode("utf-8").hex() + '.json', 'r') as j:
...
# tbsmovies_ldjson.to_csv('/home/lisi/MediaBiz/TBSmovies/tbsmovies_ldjson.csv', index=False)
...
for i in result:
    ...
    c = r.content
    soup = BeautifulSoup(c, 'html.parser')
    with open('/home/lisi/MediaBiz/TBSmovies/TBSmovies_drupla-settings-json/' + i.encode("utf-8").hex() + '.json', 'w') as j:
        j.write(json.dumps(
            str(soup.find_all(lambda tag: tag.name == 'script' and 
...
# for i in result:
#     try:
#         with open('/home/lisi/MediaBiz/TBSmovies/TBSmovies_drupla-settings-json/' + i.encode("utf-8").hex() + '.json', 'r') as j:
#             dict_list.append(dict(eval(json.load(j).replace('null', '"NaN"')\
#                                        .replace('true',"True")\
...
# tbsmovies_drupla_settings_json.to_csv('/home/lisi/MediaBiz/TBSmovies/tbsmovies_drupla_settings_json.csv', index=False)

# # merge both datasets and save them

# df_tbsmovies = pd.merge(tbsmovies_drupla_settings_json, tbsmovies_ldjson, how='left', on='url')

# df_tbsmovies.to_csv('/home/lisi/MediaBiz/TBSmovies/tbsmovies.csv', index=False)
